...
*/
/*
// 258 - 260. Consuming Promises with Async/Await
const getPosition = function () {
  return new Promise(function (resolve, reject) {
    navigator.geolocation.getCurrentPosition(
      position => resolve(position),
...
const whereAmI = async function () {
  try {
    ...
    // Reverse geocoding
    const rezGeo = await fetch(`https://geocode.xyz/${lat},${lng}?geoit=json`);
    if (!rezGeo.ok) throw new Error('Problem getting location error');
    const dataGeo = await rezGeo.json();
...
// Promise.allSettled. Return all results (rejected too)
Promise.allSettled([
...
//Promise.any . Rejected promises ignored. Simular Promise.race
Promise.any([
  Promise.resolve('Success'),
  Promise.reject('Error'),
  Promise.resolve('Success'),
...
// implementation using fetch - then
// createImage('img/img-1.jpg')
//   .then(img => {
//     currentImageElement = img;
//     return wait(2);
...
const loadNPause = async function () {
  try {
    ...
    imgContainer.append(err.message);
  }
};
//loadNPause();
// PART 2
